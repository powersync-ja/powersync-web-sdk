...
Deno.serve(async (req) => {
  ...
  let hexToUint8Array = function (hexString) {
    return Uint8Array.from(hexString.replace("\\x","").match(/.{1,2}/g).map((byte) => parseInt(byte, 16)));
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: String(err?.message ?? err) }, { status: 500, headers: responseHeaders }))
  }
})
